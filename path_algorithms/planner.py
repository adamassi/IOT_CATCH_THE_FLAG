...
def get_path_to_goal(start_pos, goal_pos, cube_obstacles=[]):
    ...
    # Use the validated setter; an invalid position returns an empty path instead
    # of crashing the whole robot program.
    try:
        planning_env.set_start_and_goal(
            start=[start_pos[0], start_pos[2]],
            goal=[goal_pos[0], goal_pos[2]]
        )
    except ValueError as e:
        print(f"[planner] Invalid start/goal — {e}")
        clear_dynamic_cubes()
        remove_cube_obstacle(planning_env, nember_of_added_obstacles)
        return np.array([])

    # Create the planner for the selected algorithm
    ...
    print(f"Planning path from {planning_env.start} to {planning_env.goal} using {PlannerConfig.ALGORITHM}...")
    # Execute the planning algorithm to get the path
    plan = planner.plan()

    # Return early if no path was found to avoid smoothing an empty array
    if plan is None or len(plan) == 0:
        print(f"[planner] {PlannerConfig.ALGORITHM} returned an empty plan.")
        clear_dynamic_cubes()
